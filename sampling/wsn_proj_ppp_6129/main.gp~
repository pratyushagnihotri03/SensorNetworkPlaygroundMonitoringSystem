...
plot "humidity.dat" using 1:2 axes x1y1 with lines ls 1 title "{/Arial=14 Humidity}"

set ylabel "Threshold value [VWT]"
set style line 1 lt 1 lc rgb "olive" lw 2 pt 1
set output "soil_kalanchoe.eps"
plot "soil_kalanchoe.dat" using 1:2 axes x1y1 with lines ls 1 title "{/Arial=14 Soil Moisture(Kalanchoe)}"

set ylabel "Threshold value [VWT]"
set style line 1 lt 1 lc rgb "olive" lw 2 pt 1
set output "soil_peperomia.eps"
plot "soil_peperomia.dat" using 1:2 axes x1y1 with lines ls 1 title "{/Arial=14 Soil Moisture (Peperomia)}"

set ylabel "Threshold value [PPM]"
set style line 1 lt 1 lc rgb "brown" lw 2 pt 1
set output "co2.eps"
plot "co2.dat" using 1:2 axes x1y1 with lines ls 1 title "{/Arial=14 CO2}"
